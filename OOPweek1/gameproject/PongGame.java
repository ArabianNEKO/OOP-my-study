...
public class PongGame extends JFrame {
    ...
    public PongGame(String ip, int port) throws IOException {
        setTitle("Pong Game");
        setSize(1600, 900);
        ...
        gamePanel = new GamePanel(client); // ส่ง Client ให้กับ GamePanel
        
        add(gamePanel);

        client.start();  // เริ่มการเชื่อมต่อกับ Server

        setVisible(true);
    }
    public void updateGameState(String gameState) {
        // อัปเดตสถานะของเกม เช่น ตำแหน่งของ paddle, ball, คะแนน
        gamePanel.updateStateFromServer(gameState);
        gamePanel.repaint();
    }

    public static void main(String[] args) throws IOException {
        String ip = "192.168.1.1";  // เปลี่ยนเป็น IP ของ Server
        int port = 12345;          // เปลี่ยนเป็น port ของ Server
        new PongGame(ip, port);
    }
}
...
class GamePanel extends JPanel implements KeyListener, Runnable {
    ...
    public GamePanel(Client client) { // รับ Client เป็นพารามิเตอร์
        setBackground(Color.black);
        setFocusable(true);
        addKeyListener(this);
    
        paddles = new Paddle[4];
        paddles[0] = new Paddle(110, 410, KeyEvent.VK_W, KeyEvent.VK_S, client);
        paddles[1] = new Paddle(310, 410, KeyEvent.VK_R, KeyEvent.VK_F, client);
        paddles[2] = new Paddle(1260, 410, KeyEvent.VK_U, KeyEvent.VK_J, client);
        paddles[3] = new Paddle(1450, 410, KeyEvent.VK_UP, KeyEvent.VK_DOWN, client);
    ...
    public void updateStateFromServer(String gameState) {
        // แยกข้อมูลจาก gameState ที่ได้รับ
        String[] states = gameState.split(",");
    
        // อัปเดตตำแหน่ง paddle และลูกบอล (สมมติว่าข้อมูลถูกส่งมาในรูปแบบที่ชัดเจน)
        for (int i = 0; i < paddles.length; i++) {
            int paddleY = Integer.parseInt(states[i]);
            paddles[i].setY(paddleY); // ต้องมี setter method ใน Paddle สำหรับการเปลี่ยน Y
        }
    
        // อัปเดตตำแหน่งของลูกบอล
        ball.setX(Integer.parseInt(states[paddles.length])); // ลูกบอลตำแหน่ง X
        ball.setY(Integer.parseInt(states[paddles.length + 1])); // ลูกบอลตำแหน่ง Y
    
        // อัปเดตคะแนน
        ball.setScore1(Integer.parseInt(states[paddles.length + 2])); // คะแนนทีม 1
        ball.setScore2(Integer.parseInt(states[paddles.length + 3])); // คะแนนทีม 2
    }
}
...
class Paddle implements Runnable {
    ...
    private boolean gameRunning = true;     // สร้าง flag เพื่อทำให้ thread ทำงานต่อไป หรือทำให้ thread ตาย
    private Client client;                  // เพิ่มตัวแปร Client

    public void setY(int y) {
        this.y = y;
    }
    // constructor ของ Paddle
    public Paddle(int x, int y, int upKey, int downKey, Client client) {
        this.x = x;
        this.y = y;
        this.upKey = upKey;
        this.downKey = downKey;
        this.client = client; // กำหนดค่าตัวแปร client
    }
    ...
    public void keyPressed(KeyEvent e) {
        if (e.getKeyCode() == upKey) {
            yVelocity = -speed;
            // ส่งข้อมูลไปยัง Server เมื่อผู้เล่นกดปุ่มขึ้น
            client.sendGameAction("player moved up"); // แทนที่ด้วยข้อมูลที่ต้องการส่ง
        }
        else if (e.getKeyCode() == downKey) {
            yVelocity = speed;
            // ส่งข้อมูลไปยัง Server เมื่อผู้เล่นกดปุ่มลง
            client.sendGameAction("player moved down"); // แทนที่ด้วยข้อมูลที่ต้องการส่ง
        }
    }
